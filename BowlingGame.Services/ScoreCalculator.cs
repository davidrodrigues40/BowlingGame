...
public class ScoreCalculator : IScoreCalculator
{
    ...
    {
        if (frame.Rolls[1] == 10) // strike
            ProcessStrike(frame, nextFrame, nextNextFrame);
        else if (frame.Rolls[1] + frame.Rolls[2] == 10) // spare
    ...
    private static void ProcessStrike(IFrame frame, IFrame nextFrame, IFrame nextNextFrame)
    {
        ...
        if (nextFrame.Rolls[1] == 10)
            frame.Score += 10 + nextNextFrame.Rolls[1];
        else
            frame.Score = 10 + nextFrame.Rolls[1] + nextFrame.Rolls[2];
    }

    private static void ProcessSpare(IFrame frame, IFrame nextFrame) => frame.Score = 10 + nextFrame.Rolls[1];

    private static void ProcessOpenFrame(IFrame frame) => frame.Score = frame.Rolls[1] + frame.Rolls[2];

    private static void ProcessNinthFrame(IFrame frame, IFrame nextFrame)
    {
        if (frame.Rolls[1] == 10)
            frame.Score = 10 + nextFrame.Rolls[1] + nextFrame.Rolls[2];
        else if (frame.Rolls[1] + frame.Rolls[2] == 10)
            frame.Score = 10 + nextFrame.Rolls[1];
        else
            ProcessOpenFrame(frame);
    }
    ...
    {
        if (frame.Rolls[1] == 10)
            frame.Score = 10 + frame.Rolls[2] + frame.Rolls[3];
        else if (frame.Rolls[1] + frame.Rolls[2] == 10)
            frame.Score = 10 + frame.Rolls[3];
        else
            ProcessOpenFrame(frame);
    }
}